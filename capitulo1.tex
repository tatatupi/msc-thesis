...
We wish to estimate the state vector $x(t)$ and its covariance recursively, at regularly spaced time intervals $T$, given their initial values ($x(0)$, $P(0)$), the process (\ref{eq:prob_process})  and observation (\ref{eq:prob_obs}) models, the input or control signals, $u(iT): iT \leq kT$, and the set of past observations, $y(t_k): t_k - \delta_{k} \leq kT$. The knowledge of the time intervals $h_{k-1}: t_k - \delta_{k} \leq kT$ is also taken into consideration when time-stamp information is available. We assume that the average time interval of observations $1/\lambda_{h}$ is greater than or equal to $T$ by a factor $\alpha\geq1$, such that $1/\lambda_{h}=\alpha T$.
