...
Figure~\ref{fig:stepResponse} shows the step response of both modes, while the bode diagrams can be seen in Figures~\ref{fig:bode} and~\ref{fig:bodeS}
...
We simulate a pseudorandom binary sequence (PRBS) as input, with 200 samples in 2 seconds of simulation, according to Figure~\ref{fig:PRBS}

\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.6\textwidth]{Imagens/PRBS.eps}
	\caption[entrada]{200 samples of PRBS signal, used as input.}
	\label{fig:PRBS}
\end{figure}

Figures~\ref{fig:outputS1} and~\ref{fig:outputS2} show the response of the low-pass and high-pass modes, respectively, to the PRBS input signal.

\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.6\textwidth]{Imagens/outputS1.eps}
	\caption[entrada]{Output of low-pass system to the PRBS input signal}
	\label{fig:outputS1}
\end{figure}
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.6\textwidth]{Imagens/outputS.eps}
	\caption[entrada]{Output of the fourth order system to the PRBS input signal}
	\label{fig:outputS}
\end{figure}

The linear system is discretized by:

\begin{equation}
\begin{split}
\textbf{A}_d & = e^{\textbf{A}T} \\
\textbf{B}_d & = \textbf{A}^{-1}(\textbf{A}_d - \textit{I})\textbf{B} \\
\textbf{C}_d & = \textbf{C} \\
\textbf{D}_d & = \textbf{D}
\end{split}
\end{equation}
...
Discrete-time Kalman filter is used for estimation. Realizations of the estimated states and true values for both scenarios, with and without time-stamp, for a signal-to-noise ration in the observations of $40$ dB are shown in Figures~\ref{fig:StatesWithTimeStamp} and~\ref{fig:StatesWithoutTimeStamp}.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{Imagens/StatesWithTimeStamp.eps}
	\caption[entrada]{Estimated (red) and true (black) states considering time-stamp in the estimation algorithm.}
...
\begin{figure}[H]
	...
	\caption[entrada]{Estimated (red) and true (black) states not considering time-stamp in the estimation algorithm.}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{Imagens/LinearJEvolutiona.eps}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{Imagens/linearSamp.eps}
	\caption[entrada]{Root mean square error variation, as a function of $\lambda$ for both KF algorithms, considering and not considering time-stamp}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{Imagens/linearUsamp.eps}
	\caption[entrada]{Root mean square error variation, as a function of $\alpha$ for both KF algorithms, considering and not considering time-stamp}
	\label{fig:linearUSamp}
\end{figure} 
