...
We simulate 5 seconds of flight, considering an input profile $\delta_a$ according to~\ref{fig:AC_input} and a step size of $\delta t_{sim} = 10^{-5}$. Irregular sampling time intervals $h_k$ are generated in the same way as for the unicycle system. Figure~\ref{fig:AC_realization} shows all three system states evolution in time (in blue solid lines) and the SDKF estimates (in red dashed lines). For the third states, which is observed, the measurements are represented by the green dots

\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.8\textwidth]{Imagens/AC_input.eps}
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.8\textwidth]{Imagens/AC_realization.eps}
...
\begin{figure}[!htb]
	...
	\caption[entrada]{Temporal cut from 0 to 0.2 seconds, for a realization of pitch rate about the center of mass RMSE of both estimators, with and without time-stamp. Vertical dashed lines on $x$ axis match the measurement sampling instants $t_k$. Black dots represent the regular estimation instants.}
	\label{fig:AC_J}
\end{figure}
