...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.5\textwidth]{Imagens/Nonholomonic-robot.pdf}
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.8\textwidth]{Imagens/AC_schematic.pdf}
...
We simulate 5 seconds of flight, considering an input profile $\delta_a$ according to~\ref{fig:AC_input} and a step size of $\delta t_{sim} = 10^{-5}$. Noise $\omega(t)$ was generated as white gaussian with unity intensity, with a step size of 0.1 seconds and interpolated for all simulation data points. Figure Irregular sampling time intervals $h_k$ are generated in the same way as for the unicycle system. Figure~\ref{fig:AC_realization} shows all three system states evolution through simulation time, the SDKF estimates and the noisy measurements for the observed state, considering SNR\textsubscript{y} $= 20$ dB, $\lambda = 0.1$ s and $\alpha = 2$.
