...
\todo[inline]{Falta traduzir seção 4.2 e incluir revisão das métricas de performance (teste chi quadrado e rmse)}
...
The Bayesian recursive solution described by proposition~\ref{prop:bayes_solution} enables the computation of the optimal estimation of state vector $x_k$. However, its implementation is impossible, since it relies on mathematical integrations and as time evolves, the observation sequence grow indefinitely. The sequential algorithm proposed by \citep{Kalman1960} solves that problem adding a restriction in the system assumptions, that is linearity and Gaussianity. 
...
\begin{quote}
\textit{Data assimilation step}: Using the measurement vector $y_k$, update estimations from previous step with (\ref{eq:assim_state}) and (\ref{eq:assim_cov}), obtaining estimates $\hat{x}_{k|k}$ and $P^{xx}_{k|k}$.
\end{quote}
...
Adaptations to the Kalman filter have been proposed in the literature. The extended Kalman filter (EKF), for example, linearizes the system around the state estimates, to achieve approximations through algorithm~\ref{alg:KF}. A different approach is to approximate the statistics of the posterior PDF, instead of the model. The unscented Kalman filter (UKF) \citep{Julier2004} performs statistical approximations via the unscented transformation (UT), addressing many of the issues that appear in the EKF implementation, such as the necessity of differentiability, the performance loss due to systems that are poorly approximated by linearization and computational efficiency. Therefore the UKF is the chosen method for the non-linear system state estimation simulated in Chapter~\ref{cap5} and will be further discussed in this section.

Based on the Monte Carlo approach to statistical approximation, the UT defines a set of samples $\chi \triangleq [\chi_0,...,\chi_{2n}] \in \mathbb{R}^{n\times (2n+1)}$, also referred to as sigma points, to approximate the mean $\bar{x}$ and covariance $\bar{P}^{xx}$ of the random variable being estimated, by
...
For this study we will adopt a simplified version of the UKF, with fixed parameters. Assuming that the non-Gaussian distribution is symmetric, the choice of the sigma points is done deterministically by
...
\noindent
\textit{Forecast step}: Calculate sigma points based on last estimation and use them to forecast state estimate via the observation model and the respective covariance.
\begin{align}
...
\end{align}
Calculate new sigma points based on state estimate forecast and propagate them to an estimated observation vector using the observation model. Calculate observation and state vectors covariance and the cross-covariance.
...
\hat{x}_{k-1|k-1} 	&= \hat{x}_{k|k-1} + K_k(y_k - \hat{y}_{k|k-1}),	 \\
P^{xx}_{k|k}		&= P^{xx}_{k|k-1} - K_k (P^{yy}_{k|k-1}) K_k^T.
\end{align}
...
If the estimator knows the exact time $t_k$ that measurements were taken in a global timescale, \textit{data assimilation} steps can be performed at the correct time instants. For that, the process model defined by (\ref{eq:prob_process}) is discretized at variable time intervals $\delta t_j^*$, yielding
...
\noindent
where $\tilde{x}_{k|k}$ is the estimation error at time instant $k$. Condition (\ref{eq:consistency1}) is called \textit{unbiasedness} requirement, whereas (\ref{eq:consistency1}) refers to \textit{covariance matching}. For the non-linear case, these conditions cannot be fully met, since the \textit{posterior} PDF is approximated to a Gaussian. Thus, the closer they get to them, the more consistent are the filter results.

In this study we will use metrics that measure both consistency conditions. According to Bar-Shalom, in order to test both aforementioned conditions, the consistency criteria metrics for state estimation must certify: that state estimate errors are zero mean and compatible with the estimated state covariance; that innovations are also zero mean and compatible with their respective covariances; and that innovations are white. We will adopt two tests proposed by him that attests all conditions simultaneously, that is the normalized (state) estimation error squared (NEES) and normalized innovation squared (NIS) tests. We first defined NEES and NIS as
...
Under the linear and Gaussian assumption, we formulate a hypothesis test, under which the null hypthesis $H_0$ that the filter is consistent, both NEES and NIS are chi-squared distribution, with $n_x$ and $n_y$ degrees of freedom, respectively, that is $n_x$ is the dimension of the state vector and $n_y$ is the dimension of the observation vector. The expected value of a RV that is chi-squared distributed is equal to its degree of freedom, that is $E[NEES_k] = n_x, \ \forall k > 1$ and  $E[NIS_k] = n_y, \ \forall k > 1$. 

We adopt single-run tests, thus for every estimation, we test if $H_0$ is accepted if both NEES and NIS at time instants $k$ are within a certain interval, considering the accepted region as 
...
Additionally, since we are using simulated systems, the root mean square error (RMSE) of the system states will also be assessed, as an accuracy performance index, given by


\begin{equation}\label{eq:rmse}
J = \frac{ \sum_{i=1}^N \sqrt{(\hat{x}_{k|k}-x_{k})^2}}{N}
\end{equation}
