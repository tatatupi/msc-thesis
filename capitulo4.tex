...
The goal of the estimator is to statistically infer the value of the sates of a system, say a random vector $X$, from the conditional PDF $\rho(X|Y)$, that is given the noisy observation vector $Y$. This inference is performed by~\citep{Candy2008}

\begin{equation}
\hat{X} \triangleq \operatorname{arg}  \underset{X}{\operatorname{max}} \ \hat{\rho}(X|Y)
\end{equation}

where...

Finding this conditional probability is the complete solution to the estimation problem and $\hat{X}$ is the optimal state estimate. (citar bruno)
