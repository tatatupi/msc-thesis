...
A recursive Bayesian solution to the state estimation problem, considering that the system evolves according a Markov process is presented in Proposition~\ref{prop:bayes_solution}. But first, we need to define two lemmas.
...
\rho(x_{k+1}|(y_1,...,y_k)) &= \int_{\mathbb{R}^n}\rho(x_{k+1}|x_k) \rho(x_k|y_1,...,y_k)dx_k \label{eq:prop_xk1},
\end{align}
	

\noindent
where $k \in \mathbb{N}$, $\rho(y_k|x_k)$ is the \textit{likelihood density}, $\rho(x_k|(y_1,...,y_{k-1})$ is the \textit{prior density}, defined before the latest measurement, $\rho(x_{k+1}|x_k)$ is the \textit{transition density} that models the evolution of $x_k$ and $\rho(y_k|(y_1,...,y_{k-1}))$ is the \textit{evidence}, also referred to as normalizing factor, or marginalization. The evidence term is commonly presented as a constant~\citep{Sarkka2013} and can be computed by the Chapman-Kolmogorov equation from Lemma~\ref{lemma:chapman}
...
In the next subsection we derive the optimal linear estimator for the case where the system, apart from satisfying Markov properties defined by Lemma~\ref{lemma:markov}, has also Gaussian uncertainties.
...
Now consider that the \textit{posterior} density function from a previous step is given by
...
\noindent
the Bayesian recursive solution to such a system is defined by forecasting and updating the conditional PDF, by
...
Now we combine the \textit{forecast steps} from (\ref{eq:prop_xk1}) and from (\ref{eq:sol_xk}), using the process model from (\ref{eq:rho_process}), the previous estimate from (\ref{eq:previous}) and the identities from properties~\ref{prop:marginal}, yielding
