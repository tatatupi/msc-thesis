...
\noindent	
where $t_{k+1} = t_k + h_k$, $\in \mathbb{R}$ are discrete time instants as in (\ref{eq:time_interval}), separated by random time intervals $h_k \in \mathbb{R}$ and $k \in \mathbb{N}$ is the discrete-time index. Process noise PDF is given by $\rho(w(t_k)) = \mathcal{N}(0,Q_\textrm{d})$ and subscript $\textrm{d}$ is short for \textit{discretized}. The matrices from the discrete-time representation are given by
