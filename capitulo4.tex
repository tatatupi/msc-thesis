...
In this chapter, the field of sensor fusion under irregular sampling is resumed to the problem of estimating the states of a system with a known process model, that is observed by aperiodically sampled measurements. We begin with a review of the adopted algorithm, that is the Kalman Filter, which is the most common approach to probabilistic data fusion. The nonlinear extension based on the unscented transform is also explored.
...
\section{Discrete-Time Representation of Sampled-Data Systems}
...
A system dynamics defined by (\ref{eq:prob_process2}) can be discretized, by replacing the differentiation by differencing, via numerical methods. For this study we use the fourth order Runge-Kutta method~\citep{Suli2003}, where
...
For the linear time-invariant case (LTI), which state space representation is given by	

\begin{align}
\dot{x}(t) &= Ax(t) + Bu(t) + Gw(t),   \label{eq:prob_LTI} \\ 
y(t_k) &= Cx(t_k)+Du(t_k) + Qv(t_k), \label{eq:prob_LTIobs}
\end{align}

if u t is piecewise constant there is an exact solution to the discretization problem given by
 \citep{Chen1999}
