...
\noindent
where $f\!\!: \mathbb{R}^n \times \mathbb{R}^p \times \mathbb{R}^q \times \mathbb{R^+} \rightarrow \mathbb{R}^n $ and $g\!\!: \mathbb{R}^n \times \mathbb{R}^r \times \mathbb{R^+} \rightarrow \mathbb{R}^m $ are, respectively, the process and observation models, $k \in \mathbb{N}$ is the discrete-time index, $t_k$ represents the irregular time instants, $x(t) \in \mathbb{R}^n$ the state vector,  $u(t) \in \mathbb{R}^p$ is the input vector and $w(t)$ is the process noise vector, all at continuous-time $t$, $y(t_k) \in \mathbb{R}^m$ represents the irregularly sampled measurement sequence and $v(t_k) \in \mathbb{N}^m$ the corresponding measurement noise vector. Initial condition $x_0$ is assumed to be known.

A continuous-time system dynamics defined by (\ref{eq:prob_process2}) can be discretized, by replacing the differentiation by differencing, via numerical methods. For this study we use the fourth order Runge-Kutta method~\citep{Suli2003}, where
...
\noindent
where $f_d(\cdot)$ is given by (\ref{eq:runge_kutta})-(\ref{eq:runge_kutta2}). Note that even if the nonlinear dynamics is time-invariant, that is $f(\cdot)$ does not depend directly on $t_k$, its discrete counterpart will be time-varying if the system is irregularly sampled, since there is a time-step dependent factor outside $f(\cdot)$ for each $k_n$, $n=[1,2,3,4]$.

For the linear time-invariant case (LTI), the continuous state space representation is given by	

\begin{align}
\dot{x}(t) &= Ax(t) + Bu(t) + w(t),   \label{eq:prob_LTI} \\ 
y(t_k) &= Cx(t_k) + v(t_k), \label{eq:prob_LTIobs}
\end{align}

\noindent
where matrices $A \in \mathbb{R}^{n\times n}$, $B \in \mathbb{R}^{n\times p}$ and $C \in \mathbb{R}^{m\times n}$ are known matrices, $\rho(w(t)) = \mathcal{N}(0,Q)$ and $\rho(v(t_k)) = \mathcal{N}(0,R)$ are process and measurement noises, respectively, with covariance matrices $Q \in \mathbb{R}^{n\times q}$ and $R \in \mathbb{R}^{m \times r}$. If input $u(t)$ is digitally generated at the same rate than observations are taken, and followed by a digital-to-analog converter, then it is piecewise constant. That is, $u(t)=u(t_k)$, for $t_k \leq t < t_{k+1}$. In such cases, there is an exact solution to the discretization problem of (\ref{eq:prob_LTI}), given by \citep{Chen1999}

\begin{align}
x(t_{k+1}) &= A_\textrm{d} x(t_k) + B_\textrm{d} u(t_k) + w(t_k), \label{eq:prob_process_d}\\
y(t_k) &= C_\textrm{d} x(t_k) + v(t_k), \label{eq:prob_obs_d}
\end{align}

\noindent	
where $\rho(w(t_k)) = \mathcal{N}(0,Q_\textrm{d})$ and subscript $\textrm{d}$ is short for \textit{discretized}. The matrices from the discrete-time representation are given by

\begin{align}
A_\textrm{d} &= e^{A h_k}, \\
B_\textrm{d} &= \left( \int_{\tau=t_k}^{t_{k+1}}e^{A \tau}\delta \tau \right)B, \\
C_\textrm{d} &= C, \\
Q_\textrm{d} &= \int_{\tau=t_k}^{t_{k+1}}e^{A \tau}Qe^{A^T \tau}\delta \tau,
\end{align}

\noindent
where $h_k = t_{k+1}-t_k$, $\in \mathbb{R}$ as in (\ref{eq:time_interval}), is the variable step size, defined by the random time intervals. Note that, once again, even for the time-invariant case, the matrices from the discrete-time system depend on time intervals, thus will be time-variant if they vary throughout time.

Usually, the covariance matrices $Q$ and $R$ of the noise vectors, that represent the noise level, are better expressed  by a signal-to-noise ratio (SNR) quantity, which is a more directly related measure to how contaminated is a signal. It is basically a relation between the power level of the signal and of the noise, that is
...
\begin{equation}
SNR_{\textrm{dB}} = 10 \log_{10}\frac{P_{\textrm{signal}}}{P_{\textrm{noise}}},
\end{equation}
...
\noindent
where $\hat{x_k}$ is the estimated value of $x_k \in \mathbb{R}^n$, $\hat{\rho}(x_k|(y_1,...,y_k))$ is the estimated posterior PDF of $x_k$ given the observation sequence, and $E[(\hat{x}_k-x_k)^T(\hat{x}_k-x_k)|(y_1,...,y_k)]$ is the variance of the random vector $x_k$, given the observation sequence. MAP can be interpreted as the bayesian approach to \textit{maximum likelihood} (ML) estimation, whereas MMSE is the counterpart of the \textit{least squares} (LS) estimator \citep{Bar-Shalom2001}. 
...
\begin{prop} \label{prop:joint}
	...
	\rho(y) 		&= \mathcal{N}(y;\ b,B) \\
	\rho(x|y)  	&= \mathcal{N}(x; \ a + CB^{-1}(y-b),\ A - C B^{-1} C^T ) \\
	\rho(y|x)  	&= \mathcal{N}(y; \ b + C^T A^{-1}(x-a),\  B - C^T A^{-1} C)
	\end{align}
...
\section{Simulation Scenarios Design}\label{sec:scenarios}

To evaluate performance and consistency effects of the aperiodic sampling in state estimation, we study three scenarios: variation of measurement SNR; variation of $\lambda$; and variation of $T$. First scenario intends to study how the effects vary for higher noise levels in the observation model. The second scenario assess how the average sampling rate of measurements affect the degradation in performance when time-stamp is and is not considered in state estimation. In the last simulation scenario we study the behavior of the effects when the regular estimation time interval distances itself from the average sampling rate of the observations.

Since estimation is performed in the presence of random quantities, simulations need to be performed throughout several runs, so that confidence intervals can be constructed towards more consistent conclusions. Average values for accuracy $J$ and consistency $NEES$ and $NIS$ performance indexes are calculated with corresponding $95\%$ confidence intervals.
