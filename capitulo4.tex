...
Note that the system described in Section~\ref{sec:problem_form} is Markovian, since transition model given by (\ref{eq:prob_process}) and the observation model given by (\ref{eq:prob_obs}) follow the definitions (\ref{eq:prop_xk}) and (\ref{eq:prop_xk1}), respectively.
...
\noindent
where $k \in \mathbb{N}$, $\rho(y_k|x_k)$ is the \textit{likelihood distribution}, $\rho(x_k|(y_1,...,y_{k-1})$ is the \textit{prior distribution}, defined before the latest measurement, $\rho(x_{k+1}|x_k)$ is the \textit{transition distribution} that models the evolution of $x_k$, $\rho(y_k|(y_1,...,y_{k-1}))$ is the \textit{evidence}, also referred to as normalizing factor, given by

\begin{equation}
\rho(y_k|(y_1,...,y_{k-1}) = \int_{\mathbb{R}^n} \rho(y_k|x_k)\rho(x_k|(y_1,...,y_{k-1})) dx_k,
\end{equation}

The algorithm is initiated by $\rho(x_0|(y_1,...,y_{k-1}) = \rho(x_0)$ and recursion is achieved by introducing (\ref{eq:prop_xk}) in (\ref{eq:prop_xk1})
\end{propo}
...
From Lemma~\ref{lemma:markov}, given current state $x_k$, $y_k$ is independent of past states, thus the first term of the dividend becomes $\rho(y_k|x_k)$. Additionally, Bayes' rule in the second term yields
