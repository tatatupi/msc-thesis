...
We continue this chapter advocating for the benefits of sensor fusion in Section \ref{sec:motivation}, describing its motivations and advantages. In Section \ref{sec:classification}, some of the taxonomy and classifications used in the area are discussed. We end the chapter with a deeper discussion of the aspects of data fusion related to imperfect data.
...
\begin{table}[!htb]
	...
	\caption[Direct mathematical parallels between single-sensor, single-target and multi-sensor, multi-target statistics]{Direct mathematical parallels between single-sensor, single-target and multi-sensor, multi-target. Adapted from \citep{Goodman1997, Mahler2004}}
...
Mathematically speaking, random sets are random elements whose values are sets. A random set $U$ is a finite set, whose power set $\mathcal{P}(U)$ is composed of elements described by some specific probability law. In other words, let
...
\noindent
where $B_{*}(X)$ and $B^{*}(X)$ are the \textit{B-lower} and \textit{B-upper} approximation of $X$, respectively and $BN_B(X)$ is the approximated $X$, also called as \textit{B-boundary} region. If the boundary region is empty, set $X$ is crisp or exact with respect to $B$, whereas if it is not empty, set $X$ is rough with respect to $B$. Figure~\ref{fig:rough} illustrates an approximate space in which the the universe is partitioned into elementary squares and the subset $X$ is approximated by the best upper and lower bounds.

\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.75\textwidth]{Imagens/rought_set.pdf}
	\caption{Approximation of a rough set by lower and upper crips sets.}
	\label{fig:rough}
\end{figure}
...
In this chapter, sensor fusion literature is reviewed. We explore the discussion about performance improvement from the combination of information from multiple sources, that have been drawing attention for many decades. The main reasons for the evolution of sensor fusion as a field of science are presented in a comprehensive way, divided by the expected advantages in data authenticity and data availability. There are mixed benefits from exploiting information from heterogeneous and homogeneous sources. We continue with the definition and classification of sensor fusion approaches. Four main taxonomies are reviewed: classification based on sensor interaction; the input/output model and three fusion levels; different sensor network architecture designs; and the categorization based on data challenge hierarchy. The last approach is specially interesting, since it relates to the core of this study: challenges presented by data. Algorithms that deal with the many aspects of data imperfection are summarized in Table~\ref{tab:methods}, such as: probabilistic; evidential belief; fuzzy logic; possibilistic; random set; and rough set theory. Hybrid methods have also being researched in the field.
