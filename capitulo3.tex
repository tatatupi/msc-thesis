...
It is possible to combine fuzzy theory with DS evidential reasoning to handle the imperfections that both approaches are good at, that is uncertainty, ambiguity and vagueness all together.

\subsubsection{Possibilistic}

13 years after developing the mathematical framework of fuzzy information, Zadeh introduced the concept of possibility theory \citep{Zadeh1978}, using fuzzy sets as its basis. According to him, fuzzy sets are to possibility theory what measures are to probability. 

A membership function $\mu_A(x)$ of a fuzzy set $A$ of a universe of discourse $X$ can be interpreted as the compatibility of $x$ with the concept labeled as $A$. Letting $U$ be a variable that takes values in $X$, the \textit{possibility function} of $x$ associated with $U$, denoted by $\pi_x$ is defined to be equal to the membership function of $A$, that is $\pi_X(u) \triangleq \mu_F(x)$. The interpretation, however, is that the closer $\pi_X(u)$ is to the unity, the more plausible that value is to be true.

Another way to understand possibility functions is in its comparison to probability distributions. In the information "Mary has failed a few times in Digital Control course", let us consider $X$ as the number of times in $U = {1, 2, 3...}$. The membership function $\mu_F(x)$ can model how close to "a few times" is the fuzzy variable $x$. From such model, we also define the possibility function $\pi_X(u)$ that can be interpreted as how possible it is that Mary has failed $u$ times in that course. Another way to model such lack of information in data is by a probability function $\rho_X(u)$, as how likely it is for Mary to fail $u$ times. Let us consider that an educated set of criteria was employed to define the values of both distributions as shown in Table~\ref{tab:possibility}
