...
Researchers in the field made additional efforts to categorize the fusion techniques, using different approaches. One of the earliest attempts comes from \citep{Durrant1988}, where he considered the dynamic use of information in the fusion processes, creating a so called dependence model, which grouped sensor fusion in three categories: competitive, complementary and cooperative. The first type of interaction occurs when multiple sensors measures the same properties, also known as redundant architecture. The second one refers to different types of sensors measuring different information about the same object or feature. And the last category happens when the desired information is only made available by the joint use of data from more than one sensor. A illustrative schematic is presented in Figure~\ref{fig:cat_input}
...
Architecture \citep{Castanedo2013}
