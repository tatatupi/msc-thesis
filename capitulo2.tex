...
There are techniques to handle all the irregularities discussed in this chapter and perform an effective state estimation. Provided that the estimator acknowledges the exact time the measurements were taken, there are many studies that propose different algorithms with interesting results. For some examples, we encourage the reader to check the references from each subsection in this chapter.
