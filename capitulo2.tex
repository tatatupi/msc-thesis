...
\section{Approaches for the Fusion of Imperfect Data}
...
\noindent
where $x_j \in \mathbb{R}$, $\forall j = 1\textrm{,...,}n$ is the $j^{\textrm{th}}$ input variable, $A_{i,j}$ is the $j^{\textrm{th}}$ antecedent fuzzy set for the $i^{\textrm{th}}$ rule, $C_i$ is the consequent fuzzy set for the $i^{\textrm{th}}$ rule, $f_i(X)$ is the output polynomial with respect the the input vector $X$. 
