...
\begin{figure}[!htb]
	...
	\caption[Entradas]{Aperiodic sampling categories: (a) noisy sampling over periodic intervals, with a Gaussian random variable added to expected time instants; and (b) sampling instants modeled as a stochastic process, with time intervals characterized by an exponential random variable (cumulative distribution functions are shown, for different $\lambda$ parameter value.)}
	\label{fig:aper-samp}
\end{figure}
...
The measurement model of a linear system with aperiodic sampling can be modeled as
...
\noindent
where $t_k$ are the sampling time instants and matrix $H(t_k)$ is time-varying, if derived from the discretization of a continuous time system.
